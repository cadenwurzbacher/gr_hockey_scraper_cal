"""Lakeshore Sports Centre scraper.

Schedule is in a published Google Sheet (Monthly tab), same format as Griff's/Holland:
- Row 0: date range
- Row 2: Start Date, Space, Event Name, Time
- Rows 3+: Start Date (e.g. "Sun, Mar 22"), Space, Event Name, Time ("1:30 PM - 2:50 PM")
"""
...
SHEET_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQaElWdDeT_9Eogt8EaWa0ZHgwW4hgcP2K0mzLdyOGl1h0swHYD2eJvbweNAyH9FZc2QkBzlwdl74l0/pub?gid=433713822&single=true&output=csv"
...
def _is_snp_or_open_hockey(title: str) -> bool:
    ...
    if " vs " in title.lower():
        return False
    return is_stick_and_puck_or_open_hockey(title)
...
def scrape() -> list[Event]:
    """Scrape Lakeshore Sports Centre from published Google Sheet."""
    try:
    ...
    for row in rows[3:]:
        ...
        if not date_s or not name or not time_s:
            continue
        if not _is_snp_or_open_hockey(name):
            continue
        ...
        events.append(Event(
            ...
            url="https://lakeshoresportscentre.net/",
